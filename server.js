...
db.serialize(() => {
    db.run(`CREATE TABLE IF NOT EXISTS clientes (
        ...
        email TEXT, endereco TEXT, cidade TEXT, observacoes TEXT
    )`);
    try { db.run(`ALTER TABLE clientes ADD COLUMN email TEXT`); } catch(e){}
    try { db.run(`ALTER TABLE clientes ADD COLUMN endereco TEXT`); } catch(e){}
    try { db.run(`ALTER TABLE clientes ADD COLUMN cidade TEXT`); } catch(e){}
    try { db.run(`ALTER TABLE clientes ADD COLUMN observacoes TEXT`); } catch(e){}

    db.run(`CREATE TABLE IF NOT EXISTS produtos (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        descricao TEXT, preco REAL, estoque INTEGER,
        categoria TEXT, unidade TEXT, estoque_minimo INTEGER, fornecedor TEXT
    )`);
    try { db.run(`ALTER TABLE produtos ADD COLUMN categoria TEXT`); } catch(e){}
    try { db.run(`ALTER TABLE produtos ADD COLUMN unidade TEXT`); } catch(e){}
    try { db.run(`ALTER TABLE produtos ADD COLUMN estoque_minimo INTEGER`); } catch(e){}
    try { db.run(`ALTER TABLE produtos ADD COLUMN fornecedor TEXT`); } catch(e){}

    db.run(`CREATE TABLE IF NOT EXISTS vendas (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        cliente_id INTEGER, data TEXT, total REAL,
        status_pagamento TEXT DEFAULT 'Pendente',
        FOREIGN KEY (cliente_id) REFERENCES clientes(id)
    )`);
    try { db.run(`ALTER TABLE vendas ADD COLUMN status_pagamento TEXT DEFAULT 'Pendente'`); } catch(e){}

    db.run(`CREATE TABLE IF NOT EXISTS itens_venda (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        venda_id INTEGER, produto_id INTEGER, quantidade INTEGER, preco_unitario REAL,
    ...
    db.run(`CREATE TABLE IF NOT EXISTS servicos (
        ...
        categoria TEXT, duracao_estimada TEXT, profissional TEXT
    )`);

    db.run(`CREATE TABLE IF NOT EXISTS pagamentos (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        venda_id INTEGER, data_pagamento TEXT, valor_pago REAL,
...
app.put('/atualizar-cliente/:id', (req, res) => {
    const { id } = req.params;
    const { nome, cpf, telefone, email, endereco, cidade, observacoes } = req.body;
    db.run(`UPDATE clientes SET nome=?, cpf=?, telefone=?, email=?, endereco=?, cidade=?, observacoes=? WHERE id=?`,
        [nome, cpf, telefone, email, endereco, cidade, observacoes, id], (err) => {
...
app.put('/atualizar-produto/:id', (req, res) => {
    const { id } = req.params;
    const { descricao, preco, estoque, categoria, unidade, estoque_minimo, fornecedor } = req.body;
    db.run(`UPDATE produtos SET descricao=?, preco=?, estoque=?, categoria=?, unidade=?, estoque_minimo=?, fornecedor=? WHERE id=?`,
        [descricao, preco, estoque, categoria, unidade, estoque_minimo, fornecedor, id], (err) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
});
...
app.post('/finalizar-venda', (req, res) => {
    ...
    db.run(`INSERT INTO vendas (cliente_id, data, total, status_pagamento) VALUES (?,?,?,'Pendente')`,
...
app.get('/listar-vendas', (req, res) => {
    const sql = `SELECT v.id, v.data, v.total, v.status_pagamento, c.nome as nome_cliente
                 FROM vendas v INNER JOIN clientes c ON v.cliente_id = c.id
                 ORDER BY v.id DESC`;
    db.all(sql, [], (err, rows) => {
        if (err) return res.status(500).json(err);
        res.json(rows);
...
app.post('/registrar-pagamento', (req, res) => {
    ...
    db.run(`INSERT INTO pagamentos (venda_id, data_pagamento, valor_pago, metodo_pagamento, observacoes) VALUES (?,?,?,?,?)`,
...
app.get('/listar-pagamentos', (req, res) => {
    const sql = `SELECT p.*, v.total as total_venda, c.nome as nome_cliente
                 FROM pagamentos p
